...
def get_max_neighbors_mask(
    ...
    arr = distance_sort.cpu().numpy()
    order = np.argsort(arr, axis=1, kind='stable')  # stable per-row
    sorted_arr = np.take_along_axis(arr, order, axis=1)
    distance_sort = torch.from_numpy(sorted_arr).to(distance_sort.device)
    index_sort = torch.from_numpy(order).to(distance_sort.device)
